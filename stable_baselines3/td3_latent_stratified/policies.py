...
from stable_baselines3.common.utils import get_pca_layer, get_decoder_stratified
...
class Actor(BasePolicy):
    ...
    def __init__(
        ...
        self.step_counter = 0
        self.decoders = []
        self.decoders = get_decoder_stratified(
            path_to_dir=f'./pca/pca_results/{self.env_id}/stratified',
    ...
    def forward(self, obs: th.Tensor, deterministic=False) -> th.Tensor:
        ...
        if self.use_latent:
            decoder_index = 0 if (self.step_counter%self.horizon < 50) else 1
            # decoder_index = (self.step_counter%self.horizon)//10
            # print('strata:', decoder_index)
            mu = self.decoders[decoder_index](mu)
        mu = th.tanh(mu)

        self.step_counter += 1  # only increment during training step
        self.step_counter %= self.horizon
        return mu

    def forward_strata(self, obs: th.Tensor, stratum, deterministic=True) -> th.Tensor:
        # assert deterministic, 'The TD3 actor only outputs deterministic actions'
...
class TD3LatentStratifiedPolicy(BasePolicy):
    """
    ...
    def __init__(
        ...
        super(TD3LatentStratifiedPolicy, self).__init__(
            observation_space,
            action_space,
...
MlpPolicy = TD3LatentStratifiedPolicy


class CnnPolicy(TD3LatentStratifiedPolicy):
    """
    Policy class (with both actor and critic) for TD3.
    :param observation_space: Observation space
    :param action_space: Action space
...
class MultiInputPolicy(TD3LatentStratifiedPolicy):
    """
